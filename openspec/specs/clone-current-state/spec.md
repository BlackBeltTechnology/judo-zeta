# clone-current-state Specification

## Purpose
ETL-compatible cloning strategy for `equivalentDiscriminated()` where the first caller receives the original object and subsequent callers receive clones of its current (possibly mutated) state. This enables mutation-propagation semantics required for correct action name suffix accumulation.
