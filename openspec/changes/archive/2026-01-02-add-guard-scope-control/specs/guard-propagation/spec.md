# guard-propagation Specification Delta

## Purpose

Adds ETL-compatible per-rule guard rejection caching to avoid redundant guard evaluations for the same source element.
...
- **etl-patterns**: This capability matches ETL's `TransformationRule.rejected` behavior
- **parallel-transformation**: Thread-safe rejected set using `ConcurrentHashMap.newKeySet()`
