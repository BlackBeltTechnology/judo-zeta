# Spec: Validation Framework Documentation

## Capability: Documentation

This spec defines the comprehensive documentation for the Judo Zeta Validation Framework, including user guides, best practices, EVL comparison, and real-world examples.
...
1. **Link Validation**
   - Use `markdown-link-check` in CI pipeline
   - Fail build on broken internal links
   - Warn on external link issues

2. **Code Example Testing**
   - Extract examples to test directory
   - Compile and run as part of build
   - Ensure examples stay current with API
...
Real-world examples SHALL be based on judo-meta-esm validation rules:

- `EntityTypeValidations.java` - Entity validation patterns
- `OperationValidations.java` - Complex guard/satisfies chains
- `ConstraintNames.java` - Constant naming pattern
- `GuardMethodNames.java` - Guard method conventions
- `EsmUtils.java` - Static extension delegation
- Various `*Extensions.java` - Instance extension methods

### EVL Comparison Sources

EVL examples SHALL be sourced from:

- `validations/operation/operation.evl` - Operation validation patterns
- `validations/structure/*.evl` - Structure validation patterns
- Official Epsilon documentation for syntax reference

### Maintenance Strategy

To keep documentation current:

1. **Version Tagging** - Tag docs with framework version
2. **Change Log** - Document significant changes in docs
3. **Automated Checks** - CI validates links and examples
4. **Review Process** - Docs updates reviewed with code
5. **Living Document** - Treat docs as code, update continuously
...
- Validation framework core functionality (existing)
- Test infrastructure (recently added)
- OSGi integration (existing)
