# Spec: Resource Alias Support

**Capability**: resource-alias  
**Status**: Draft  
**Parent**: transformation-core, validation-core

## Purpose

This specification defines the requirements for resource alias support in the Zeta transformation and validation frameworks. Resource aliases enable rules to specify which registered ResourceSets to use, similar to ETL's `Model!Type` pattern.
...
Existing code using the framework MUST continue to work without changes.

#### Scenario: Existing transformation without aliases

**Given** a transformation using `getAllSource()` and `createTarget()`  
**When** upgraded to the new version  
**Then** the transformation works unchanged  
**And** default "source" and "target" aliases are used implicitly  

#### Scenario: Existing validation without aliases

**Given** a validation using `getAllInstances()`  
**When** upgraded to the new version  
**Then** the validation works unchanged  
**And** default "source" alias is used implicitly  

#### Scenario: Existing @TransformRule with sourceTypes/targetTypes

**Given** a rule using `@TransformRule(sourceTypes = {...}, targetTypes = {...})`  
**When** upgraded to the new version  
**Then** the rule works unchanged  
**And** default aliases are applied to all types  

#### Scenario: New @Transform/@To annotations work alongside existing attributes

**Given** a transformation using new `@Transform` and `@To` annotations  
**When** the transformation is executed  
**Then** elements are collected from aliased resources as specified  
**And** the behavior matches the ETL `transform ... to ...` pattern  

#### Scenario: Existing tests pass

**Given** the existing test suite  
**When** all tests are executed after the change  
