 ...
 *
 *     @Cached
 *     public Collection<DataMember> getAllDataMembers(EntityType self, MemberType memberType) {
 *         // Cached per (EntityType, MemberType) combination
 *         return self.getAttributes().stream()
 *             .filter(a -> a.getMemberType() == memberType)
 *             .collect(Collectors.toList());
 *     }
 * }
 * }
